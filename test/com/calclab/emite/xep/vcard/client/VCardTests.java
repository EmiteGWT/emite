...
public class VCardTests {
    ...
    @Before
    public void setup() {
	IPacket packet = TigaseXMLService.toPacket(VALID_VCARD);
	vCard = new VCard(packet);
    }

    @Test
    public void shouldParseAddress() {
	List<VCardAddress> addresses = vCard.getAddresses();
	assertEquals(2, addresses.size());
	VCardAddress first = addresses.get(0);
	...
	VCardOrganization organization = vCard.getOrganization();
	assertEquals("XMPP Standards Foundation", organization.getName());
	assertEquals("Org Unit", organization.getUnit());
    }
	...
	List<VCardTelephone> telephones = vCard.getTelephones();
	assertEquals(6, telephones.size());
	VCardTelephone first = telephones.get(0);
